          ...
          <div className="lg:col-span-7">
            <div className="inline-block border border-white/20 px-3 py-1 mb-6">
              <span className="text-white/65 text-xs font-mono uppercase tracking-widest">
                06 — Compromiso ambiental
              </span>
            </div>
            <h2 className="text-white mb-6">Comprometidos con la economía circular</h2>
            <p className="text-white/45 leading-relaxed text-lg">
              [Descripción — Para Hisoil la economía circular no es un eslogan: es el eje de cada
              solución que diseñamos. Creemos que cada residuo es una oportunidad, y que el sistema
              agro-industrial puede ser más eficiente y más justo con el ambiente.]
            </p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-px bg-white/10 w-full">
              {metrics.map((m) => (
                <div key={m.label} className="bg-oliva p-6 text-center">
                  ...
                  <div className="text-white/70 text-xs font-semibold mb-0.5 leading-tight">
                    {m.label}
                  </div>
          ...
          {flow.map((f, i) => (
            <div key={f.step} className="flex gap-5">
              <div className="flex flex-col items-center">
                ...
                {i < flow.length - 1 && (
                  <div className="w-px flex-1 bg-white/10 my-2" style={{ minHeight: 32 }} />
                )}
              </div>
              <div className="pb-8 flex-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {f.examples.map((ex) => (
                    <span
                      key={ex}
                      className="bg-white/5 border border-white/10 text-white/55 text-xs font-mono px-2.5 py-1 rounded-full"
                    >
                      {ex}
                    </span>
                  ))}
        ...
        <div className="border border-white/10 rounded-sm p-8 md:p-10 mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <p className="text-white/65 leading-relaxed text-sm">
              [Texto — Descripción del marco de economía circular que aplica Hisoil en sus
              proyectos: cómo se integra en el diseño de cada solución y qué beneficios genera para
              el cliente y el ecosistema.]
            </p>
            <p className="text-white/65 leading-relaxed text-sm">
              [Texto — Certificaciones ambientales obtenidas, compromisos medibles asumidos
              públicamente y alianzas estratégicas con organismos de control y entidades del
              sector.]
            </p>
          </div>
        </div>
