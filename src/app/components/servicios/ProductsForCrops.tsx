        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-12">
          {navCategories.map((cat) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-lg backdrop-blur-md border flex items-center justify-center mb-3 transition-all duration-500 ${
                    isActive ? 'bg-lima/30 border-lima/50' : 'bg-oliva/10 border-oliva/20'
                  }`}
                >
                  <Icon className={`w-5 h-5 ${isActive ? 'text-white' : 'text-oliva'}`} />
                </div>
                <div
                  className={`font-medium text-sm leading-tight mb-2 ${isActive ? 'text-white' : 'text-gray-700'}`}
                >
                  {cat.label}
                </div>
                <div
                  className={`text-xs font-mono ${isActive ? 'text-white/70' : 'text-gray-600'}`}
                >
                  {cat.count} productos
                </div>
              </button>
